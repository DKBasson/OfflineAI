...
type TabId = 'general' | 'models' | 'prompts' | 'system';
...
export function SettingsPanel() {
  ...
  const defaultId = loadSettings().defaultPromptId;

  const statusColor = (s: string) =>
    s.startsWith('Error') ? 'text-err-text' : s.startsWith('✓') ? 'text-ok' : 'text-text-muted';
  ...
  return (
    <>
      ...
      <div
        ref={panelRef}
        className="fixed top-0 right-0 bottom-0 w-[380px] z-50 flex flex-col overflow-hidden"
        style={{
      ...
      >
        ...
        {/* ── Tab bar */}
        <div className="flex px-5 mt-4 border-b border-border shrink-0">
          {TABS.map((tab) => (
            <button
              key={tab.id}
        ...
        {/* ── Tab content — key resets scroll on tab change */}
        <div key={activeTab} className="flex-1 overflow-y-auto px-5 py-5">
                ...
                <div className="settings-card-body">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <TipField label="Temperature" tip="Randomness. 0 = deterministic, 2 = very creative.">
                      <input className="settings-input" type="number" min={0} max={2} step={0.1} inputMode="decimal"
                    ...
                    <TipField label="Top P" tip="Nucleus sampling. Lower = more focused replies.">
                      <input className="settings-input" type="number" min={0.1} max={1} step={0.05} inputMode="decimal"
            ...
            <div>
              ...
              <div className="settings-card">
                ...
                <div className="settings-card-body">
                  ...
                  {pullStatus && <p className={`text-[12px] mt-1.5 ${statusColor(pullStatus)}`}>{pullStatus}</p>}
                </div>
              </div>
              ...
              <div className="settings-card">
                ...
                <div className="settings-card-body">
                  ...
                  <div>
                    ...
                    {pullImageStatus && <p className={`text-[12px] mt-1.5 ${statusColor(pullImageStatus)}`}>{pullImageStatus}</p>}
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <div className="space-y-1.5">
                      {prompts.map((p, i) => (
                        <div key={p.id} className="sp-saved-item">
                          <span className="flex-1 text-[13px] text-text-muted truncate">{p.name}</span>
                          <button
                            className={`sp-btn text-[14px] ${p.id === defaultId ? 'text-yellow-400' : 'text-text-dim hover:text-yellow-400'}`}
                            title="Set as default"
                            onClick={() => { setDefaultPrompt(p.id); setPrompts(getSavedPrompts()); }}
                          >★</button>
                          ...
                          <button className="sp-btn" title="Move down" disabled={i === prompts.length - 1}
                            onClick={() => { reorderPrompts(i, i + 1); setPrompts(getSavedPrompts()); }}>↓</button>
                          <button className="sp-btn" title="Duplicate"
                            onClick={() => { duplicatePrompt(i); setPrompts(getSavedPrompts()); }}>⧉</button>
                          <button className="sp-btn" title="Edit" onClick={() => openEditPrompt(i)}>✎</button>
                          <button className="sp-btn hover:text-err-text" title="Delete"
                    ...
                    {showNewPromptForm ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2 justify-end">
                          <button className="settings-secondary-btn" style={{ width: 'auto', padding: '7px 14px' }} onClick={resetPromptForm}>
                            Cancel
                          </button>
                          <button className="settings-save-btn-inline" onClick={handleSavePrompt}>
            ...
            <div>
              ...
              <div className="settings-card">
                ...
                <div className="settings-card-body">
                  ...
                  <button className="settings-secondary-btn mb-2" onClick={restartOllama} type="button">
                    Restart Ollama
                  </button>
                  {ollamaRestartStatus && (
                    <p className={`text-[12px] mb-2 ${ollamaRestartStatus.startsWith('Error') ? 'text-err-text' : 'text-ok'}`}>
                      {ollamaRestartStatus}
                    </p>
                  )}
                  {isLocalhost && (
